...
function setGPA() {
  ...
  if (creditSum == 0) {
    ...
    result = (sum / creditSum).toFixed(2); //到小數點第二位
  }
...
addButton.addEventListener("click", () => {
  ...
  newButton.addEventListener("click", (e) => {
    e.preventDefault(); //防止新的button 垃圾桶 送出資料
    e.target.parentElement.parentElement.style.animation = //form 動畫 縮小
...
allTrash.forEach((trash) => {
  let form = trash.parentElement.parentElement; //form
...
function handleSorting(direction) {
  ...
  for (let i = 0; i < graders.length; i++) {
    ...
    let class_number = graders[i].children[1].value; //calss number
  ...
  //取得object array後 我們可以把成績String換成Number
  ...
  //Select 可直接用JS 更改
  graders = document.querySelectorAll("div.grader"); //上面已經更動內容故重新抓一次
  for (let i = 0; i < graders.length; i++) {
    graders[i].children[3].value = objectArray[i].class_grade;
  }

  //Select 事件監聽 因為排序後的內容為innerHtml 無監聽 故補上
